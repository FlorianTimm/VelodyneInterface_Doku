...
Die abstrakte Klasse \code{VdASCIIFile} (siehe Anhang~\ref{a:vdASCIIFile.py}, \ref{a:VdASCIIFile.h} und \ref{a:VdASCIIFile.cpp}) erweitert die Klasse \code{VdFile} um Funktionen, um ASCII-Dateien zu schreiben. \code{VdTxtFile}, \code{VdObjFile} und \code{VdXYZFile} implementieren dann die eigentlichen Dateiformate. Die Klasse \code{VdTxtFile} (siehe Anhang~\ref{a:vdTxtFile.py}, \ref{a:VdTxtFile.h} und \ref{a:VdTxtFile.cpp}) schreibt hierbei die Rohdaten als einfache Textdateien (siehe Anhang~\ref{a:AusgabeTXT}). \code{VdObjFile} (siehe Anhang~\ref{a:vdObjFile.py}, \ref{a:VdObjFile.h} und \ref{a:VdObjFile.cpp}) formt die Rohdaten in lokale Koordinaten mit dem Ursprung im Standpunkt des Scanners um und speichert die Daten als OBJ-Datei (siehe Anhang~\ref{a:AusgabeOBJ}). Das Dateiformat ist für das einfache Betrachten der Daten hilfreich. Es lässt sich als Punktwolke in vielen Programmen, wie zum Beispiel \code{MeshLab}, einlesen und darstellen. Im Zusammenhang mit der Vergleichsmessung aus \autoref{c:systemueberpruefung} wurde zu\-sätz\-lich noch eine Klasse zur Implementierung von XYZ-Dateien (\code{VdXYZFile}, siehe Anhang~\ref{a:vdXYZFile.py}, \ref{a:VdXYZFile.h} und \ref{a:VdXYZFile.cpp}) erstellt. Hier werden die schon für die OBJ-Dateien aufbereiteten Daten in anderer Formatierung genutzt.
Die Daten aus der TXT-Datei können auch im Nachhinein in die anderen Dateiformate umgeformt werden. Hierfür wurde ein Konvertierungsskript erstellt (siehe Anhang~\ref{a:convTxt.py}).

\paragraph{VdSQLite (Python)}
Die Klasse \code{VdSQLite} (siehe Anhang~\ref{a:vdSQLite.py}, \ref{a:VdSQLite.h} und \ref{a:VdSQLite.cpp}) er\-mög\-licht das Speichern der Rohdaten in einer SQLite-Datenbank-Datei. Das Format bietet sich zum einfachen Übertragen und Auswerten der Daten an. Alle Daten werden in einer Datenbank-Datei gesichert. Dies erleichtert das Einladen der Daten in weitere Skripte, zum Beispiel zum Durchführen von Berechnungen.

\subsection{VdBuffer (Python)}
Die Klasse \code{VdBuffer} (siehe Anhang~\ref{a:vdBuffer.py}) übernimmt die Zwischenspeicherung der binären Rohdaten vom Laser\-scan\-ner. Über die Klasse \code{VdInterface} wird eine Socketverbindung zum UDP-Port des Laser\-scan\-ners hergestellt. Anschließend werden die Daten vom Socket in einer Variable gespeichert und re\-gel\-mä\-ßig in kurzen Intervallen als binäre Datei auf dem Speicher des Rasp\-berry Pi abgelegt. Hierzu wird bei dem ersten Datenempfang eine Verzeichnisstruktur für die Messung angelegt. Nach dem Schreiben der Datei wird ihr Pfad in die Warteschleife für \code{VdTransformer} eingetragen.
...
\begin{figure}
 \centering
 \includegraphics[width=.24\textwidth]{./img/web1.png}
 \includegraphics[width=.24\textwidth]{./img/web2.png}
 \includegraphics[width=.48\textwidth]{./img/veloweb.png}
 % Strahlengang.eps: 0x0 pixel, 300dpi, 0.00x0.00 cm, bb=0 0 3066 4000
 \caption{Weboberfläche des Skriptes (beide links) und des Laserscanners (rechts)}
 \label{img:web}
\end{figure}
...
\section{config.ini}
\label{a:config.ini}
\lstinputlisting[language=Ini]{../Quelltext/Python/config.ini}
...
\\
\\
\noindent{\underline{Hamburg, den 13. Dez. 2017~\hspace{10cm}}}\\
