...
\autoref{abb:steuerung} zeigt den Schaltplan des entwickelten Steuermodules. Dieses bietet mit drei Leuchtdioden und 2 Tastern die Möglichkeit, im Skript später einfache Anzeigen und Eingaben zu realisieren. Hierfür wurde eine Erweiterung auf Basis des GPIO-Portes des Raspberry Pi aufgebaut. Die zwei Taster sind über die beiden GPIO-Pins 18 und 25 erreichbar. Ohne Betätigung werden die Eingänge über internen die Pull-Up-Widerstände (\todo{R?}) auf ein High-Level gezogen. Durch Drücken des Tasters wird die Spannung über die Widerstände \todo{Widerstände} auf ein Low-Level gezogen, welches durch das Pythonskript zur Laufzeit ausgelesen werden kann. Der Widerstand \todo{R?} dient zur Strombegrenzung und als Sicherheit, falls die GPIO-Pins falsch geschaltet werden.
...
\begin{figure}
 \centering
 \includegraphics[width=0.7\textwidth]{img/platine.pdf}
...
Die Klasse \code{VdInterface} (siehe Anhang~\ref{a:vdInterface.py}) übernimmt die Kommunikation mit dem Laser\-scan\-ner.


\subsection{VdHardware}
Die Klasse \code{VdHardware} übernimmt die Hardwaresteuerung des Raspberry Pi. Um etwas unabhängiger zu sein, stellt die Klasse einen eigenen Thread dar, der von der Klasse \code{VdAutoStart} je nach Hardware gestartet wird. 
...
\subsection{VdPoint}
Die \code{VdPoint}-Klasse stellt einen Messpunkt der Velodyne dar. Er nimmt als Attribute die Messdaten auf.

\subsection{VdDataset}
Die Klasse \code{VdDataset} nimmt eine Menge von Messdaten auf. Diese Klasse sorgt auch für das Interpretieren der binären Daten vom Laserscanner. Die Daten werden dann als \code{VdPoint}-Objekte in einer Liste gesichert. Durch die Übergabe der Daten an die Klasse \code{VdFile} können diese dann als Datei gespeichert werden.

\subsection{VdFile und Subklassen}
Bei der Klasse \code{VdFile} handelt es sich um eine abstrakte Klasse. Sie übernimmt die Speicherung der umgewandelten Daten. Die genauen Datenformate werden in Klassen festgelegt, die von ihr erben.

\paragraph{VdASCIIFile / VdTxtFile / VdObjFile}
...
\paragraph{VdSQLite}
Die Klasse \code{VdSQLite} ermöglicht das Speichern der Rohdaten in einer SQLite-Datenbank-Datei. Das Format bietet sich zum einfachen Übertragen und Auswerten von Daten an. 
\todo{Fortsetzung}
...
\lstinputlisting[language=Python, firstnumber=15, caption={Quelltext-Test}, label=pySource, firstline=15, lastline=20]{../Quelltext/velodyneInterface/vdFile.py}
...
\lstinputlisting[language=Python]{../Quelltext/velodyneInterface/vdAutoStart.py}

\section{vdBuffer.py}
\label{a:vdBuffer.py}
\lstinputlisting[language=Python]{../Quelltext/velodyneInterface/vdBuffer.py}

\section{vdTransformer.py}
\label{a:vdTransformer.py}
\lstinputlisting[language=Python]{../Quelltext/velodyneInterface/vdTransformer.py}

\section{vdInterface.py}
\label{a:vdInterface.py}
\lstinputlisting[language=Python]{../Quelltext/velodyneInterface/vdInterface.py}

\section{vdGNSSTime.py}
\label{a:vdGNSSTime.py}
\lstinputlisting[language=Python]{../Quelltext/velodyneInterface/vdGNSSTime.py}

\section{vdHardware.py}
\label{a:vdHardware.py}
\lstinputlisting[language=Python]{../Quelltext/velodyneInterface/vdHardware.py}

\section{vdFile.py}
\label{a:vdFile.py}
\lstinputlisting[language=Python]{../Quelltext/velodyneInterface/vdFile.py}

\section{vdDataset.py}
\label{a:vdDataset.py}
\lstinputlisting[language=Python]{../Quelltext/velodyneInterface/vdDataset.py}

\section{vdPoint.py}
\label{a:vdPoint.py}
\lstinputlisting[language=Python]{../Quelltext/velodyneInterface/vdPoint.py}

\section{config.ini}
\label{a:config.ini}
\lstinputlisting[]{../Quelltext/velodyneInterface/config.ini}
...
\lstinputlisting[language=Python]{../Quelltext/convBin2Obj.py}
